...
pred <- knn_forecasting(FDX_close_prices_ts, h = 12, lags = 1:2, k = 5, transform = "none")
knn_examples(pred)
...
#Put only this model to the presentation
boost.movies=gbm(Close~., data=FDX_Xtrain, 
...
abline(v=best, lty=2, col=4)
err.boost= min(err)
#This line shows us that how many trees are best fro our dataset
#Lower training and test error number of increase
...
plot(boost.movies, i.var=2, n.trees = best) #first month number
plot(boost.movies, i.var=3, n.trees = best) #
plot(boost.movies, i.var=4, n.trees = best)
plot(boost.movies, i.var=5, n.trees = best)
plot(boost.movies, i.var=6, n.trees = best)
plot(boost.movies, i.var=7, n.trees = best)
